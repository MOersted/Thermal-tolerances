...
# This script derives TTL parameters from static experiments, where time to failure (tcoma) is measured at one or more constant temperatures. An input data template is provided (static_input.csv)
...
pdf(file="TTL_from_static.pdf",width = 12,height = 8)
for(i in 1:length(groups))
  {
  tmp = static[static$group == groups[i],]
 
  #if extra sCTmax values are desired at other exposure durations (extra_t_coma), calculate these from the TTL
  if(exists("extra_t_coma"))
    {
    if(nrow(tmp)>1)
      {
      #In the case of two or more static temperatures the parameters of the TTL can be extracted from a linear regression
      ...
      #plot TTL to assess goodness of fit
      ...
      legend("bottomleft",legend = c("Observed","Predicted","TTL fit"),pch=c(21,21,NA),pt.bg = c("black","white",NA),lwd = c(NA,NA,2),col=c("black","black","grey"),bty='n')
      }
      tmp$z_pred = z
      }else{
      ...
      #plot TTL (no fit possible with only one measurement)
      plot(tmp$assay_temp,log10(tmp$t_coma),type='n',
  ...
  #if extra t_coma values are desired at other exposure temperatures (extra_sCTmax), calculate these from the TTL
  if(exists("extra_sCTmax"))
    {
    if(nrow(tmp)>1)
      {
  ...
  if(exists("ramprates"))
    ...
    if(nrow(tmp)>1)
      {
      #In the case of two or more static temperatures dCTmax can be predicted from the parameters of the TTL
      fit = lm(log10(t_coma)~assay_temp,data=tmp)
      intercept = summary(fit)$coefficient[1,1]
      slope = summary(fit)$coefficient[2,1]
      z = -1/slope
